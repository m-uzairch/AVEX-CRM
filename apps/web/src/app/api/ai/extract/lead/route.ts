...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await aiService.extractLeads(validated.content, validated.instructions);

    return NextResponse.json(result);
  } catch (error: any) {
    console.error('[API POST /api/ai/extract/lead] Error:', error);
    return NextResponse.json(
      { error: error?.message || 'Failed to extract leads from document.' },
      { status: 400 }
    );
  }
}
